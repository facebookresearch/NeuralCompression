...
class _HiFiCDiscriminator(torch.nn.Module):
    def __init__(
            ...
            latent_dimensions: typing.Tuple[int] = (220, 16, 16),
            bottleneck_depth: int = 220,
            spectral_norm=True
    ):
        ...
        self.extract_features = torch.nn.Conv2d(bottleneck_depth, 12, 3, padding=1, padding_mode="reflect")
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor]:
        ...
        predictions = self.discriminate(x).view(-1, 1)
